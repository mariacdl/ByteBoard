...
bool Dama::validar_movimiento(pair<int, int> desde, pair<int, int> para, const Tablero& tablero, bool determinar_jaque) const {
    ...
    // Verificar si el movimiento es válido (vertical, horizontal o diagonal)
    if (!(delta_x == 0 || delta_y == 0 || abs(delta_x) == abs(delta_y)))
        return false;

    // Verificar si hay obstáculos en el camino
    int paso_x = (delta_x == 0) ? 0 : delta_x / abs(delta_x);
    int paso_y = (delta_y == 0) ? 0 : delta_y / abs(delta_y);
